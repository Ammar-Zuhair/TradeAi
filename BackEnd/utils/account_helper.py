...
def get_mapping_id(account_id: int, our_pair_name: str) -> Optional[int]:
    """
    Get MappingID for a given OurPairName and account.
    ...
    db = SessionLocal()
    try:
        mapping = db.query(AccountSymbolMapping.MappingID).join(
            TradingPair, AccountSymbolMapping.PairID == TradingPair.PairID
        ).filter(
            AccountSymbolMapping.AccountID == account_id,
            TradingPair.PairNameForSearch == our_pair_name
        ...
        return mapping.MappingID if mapping else None
...
def save_trade_to_db(
    ...
    try:
        # Get MappingID
        mapping_id = get_mapping_id(account_id, our_pair_name)
        
        if not mapping_id:
            print(f"❌ Cannot save trade: No MappingID found for {our_pair_name}")
            return False
...
def update_trade_close(
    ...
    try:
        ...
        if profit_loss > 0:
            ...
            trade.TradeStatus = 3  # Closed/Neutral (Treat as closed/losing for now or add 4)
